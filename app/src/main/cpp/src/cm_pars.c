...
/*#endif*/ /*DTEX*/
		...
		{
			pCmd_t->ParseChar = cm_pars_getseq(phTTS, input); /* parsing internally stored string*/
		}
...
/* 057	MGS		11/19/1998		BATS #812 fixed say-letter mode extra spaces */
	...
	if(c == 0xb)
	{
		...
		/* Add short silence before sync, fixes unspoken k phoneme */
                // FIXME: this causes a hang on android for some reason, also breaks binary 1:1 with original if removed
 ...
 *
 */
unsigned int cm_pars_getseq(LPTTS_HANDLE_T phTTS, unsigned char inchar) {
  register int    c;
  register int    ac=0;
