...
extern MMRESULT load_dictionary_raw( void **, void **, unsigned int *, unsigned int *,
                                 char *, int, DT_HANDLE *, DT_HANDLE *,
                                 LPVOID *, MEMMAP_T);
...
//	  wcscpy(szMainDict, phTTS->dictionary_file_name);
  ...
  if(!gnInstanceCounter || phTTS->dictionary_file_name[0]) {
	nDicLoad = load_dictionary_raw( phTTS,
...
/*               comment out the abbr dictionary section for now */
    ...
    nDicLoad = load_dictionary_raw( (void **)&(pKsd_t->fdic_index[DICT_LANG]),
