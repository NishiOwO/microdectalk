...
DEFINES=(-DSIMULATOR -DHACK_ALERT -DFULL_LANGUAGE_SUPPORT -DFULL_LTS -DNO_INDEXES -DDIRECT_LTS_INPUT -DACNA -DHOMOGRAPHS -DMATH_MODE -DDIVIDED_LTS_RULES -DOUTPUT_FILE -D__linux__ -DSINGLE_LANGUAGE -DMultiple_Sample_Rates)
...
compile_arch() {
    ...
    # build objects
    toolchain/${ARCH}-linux-$MUSL-cross/bin/${ARCH}-linux-$MUSL-gcc ${DEFINES[@]} ${CFLAGS[@]} src/*.c platform/android_bindings.cpp -fPIC -static -shared -static-libgcc -o libepsonapi.so -I /usr/lib/jvm/jre-21-openjdk/include/ -I/usr/lib/jvm/jre-21-openjdk/include/linux/
