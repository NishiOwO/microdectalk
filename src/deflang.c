...
int default_lang(unsigned int lang_code, unsigned int ready_code) {
//	unsigned int flags;
	...
	if (ready_code==LANG_both_ready)
	{
		...
		while(cp != NULL_LT)
		{
			if((*cp).lang_id == lang_code)
			{
				...
				KS.typing_table = (volatile unsigned char **)(*cp).lang_typing;
				KS.error_table = (volatile unsigned char **)(*cp).lang_error;
			}
			cp = (*cp).link;
		}
#endif
